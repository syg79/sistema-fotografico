    ...
    $clientesPath = 'd:/Projetos/Excel/csv_output/Clientes.csv'
    ...
    $idxNomeEmpresa = [Array]::IndexOf($headersClientes, 'nome_empresa')
    ...
    if ($inconsistencies.Count -gt 0) {
        Write-Output "Valores da coluna 'nome_cliente' em Solicitacao.csv que não existem em Clientes.csv:"
        $inconsistencies | ForEach-Object { Write-Output $_ }
    } else {
        Write-Output "Todos os valores da coluna 'nome_cliente' em Solicitacao.csv existem em Clientes.csv."
    }
